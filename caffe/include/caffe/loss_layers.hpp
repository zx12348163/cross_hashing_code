...
class EuclideanLossLayer : public LossLayer<Dtype> {
 ...
 protected:
  ...
  inline void set_phase(const string phase) {phase_ = phase;}
...
template <typename Dtype>
class ChooselabelSoftmaxWithLossLayer : public LossLayer<Dtype> {
 public:
   /**
    * @param param provides LossParameter loss_param, with options:
    *  - ignore_label (optional)
    ...
    *    present; otherwise the loss is simply summed over spatial locations.
    */
  explicit ChooselabelSoftmaxWithLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline const char* type() const { return "ChooselabelSoftmaxWithLoss"; }
  virtual inline int ExactNumTopBlobs() const { return -1; }
  virtual inline int MinTopBlobs() const { return 1; }
  virtual inline int MaxTopBlobs() const { return 2; }
 ...
 protected:
  ...
  // internal SoftmaxLayer used to map predictions to a distribution.
  shared_ptr<Layer<Dtype> > softmax_layer_;
  /// prob stores the output probability predictions from the SoftmaxLayer.
  Blob<Dtype> prob_;
  /// bottom vector holder used in call to the underlying SoftmaxLayer::Forward
...
template <typename Dtype>
class LambdaTripletRankLossLayer: public LossLayer<Dtype> {
 public:
  explicit LambdaTripletRankLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param) {}
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  virtual inline int ExactNumBottomBlobs() const { return 2; }
  virtual inline const char* type() const { return "LambdaTripletRankLoss"; }
  virtual inline bool AllowForceBackward(const int bottom_index) const {
    return true;
  }
 ...
 protected:
  ...
  virtual void Backward_gpu(const vector<Blob<Dtype>*>& top,
      const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom);
  int LabelSimilarity(const Dtype* labels, const int i, const int j, const int dim);
  Dtype HammingSimilarity(const Dtype* labels, const int i, const int j, const int dim);
  Dtype weightTriplet(const vector<std::pair<Dtype, vector<int> > > hash_sim_vector,const int k, const int l, const Dtype best_dcg );
  int topk_;
  string type_;
  Dtype margin_;
 };
...
template <typename Dtype>
class TripletRankLossLabelLayer : public LossLayer<Dtype> {
 public:
  explicit TripletRankLossLabelLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param) {}
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  ...
  virtual inline int ExactNumBottomBlobs() const { return 2; }
  virtual inline const char* type() const { return "TripletRankLossLabel"; }
 ...
 protected:
  ...
  virtual void Backward_gpu(const vector<Blob<Dtype>*>& top,
      const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom);
  Dtype Similarity(const Dtype* labels, const int i, const int j, const int dim, const int batch_size);
 private:
  string phase_;
 
 };
template <typename Dtype>
class TripletFalseRankLossLabelLayer : public LossLayer<Dtype> {
 public:
  explicit TripletFalseRankLossLabelLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param) {}
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  ...
  virtual inline int ExactNumTopBlobs() const { return 2; }
  virtual inline int ExactNumBottomBlobs() const { return 2; }
  virtual inline const char* type() const { return "TripletFalseRankLossLabel"; }
...
template <typename Dtype>
class PairContrastiveLossLayer : public LossLayer<Dtype> {
 public:
  explicit PairContrastiveLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param){}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline int ExactNumBottomBlobs() const { return 2; }
  virtual inline int ExactNumTopBlobs() const { return 2; }
  virtual inline const char* type() const { return "PairContrastiveLoss"; }
  /**
   * Unlike most loss layers, in the ContrastiveLossLayer we can backpropagate
   * to the first two inputs.
   */
 ...
 protected:
  std::vector<std::pair<Dtype, std::pair<int, int> > > similar_pairs_;
  std::vector<std::pair<Dtype, std::pair<int, int> > > dissimilar_pairs_;
  int similarity(const Dtype* labels, int i, int j, int dim );
  virtual void Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Forward_gpu(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
class PairTripletRankLossLayer : public LossLayer<Dtype> {
 public:
  explicit PairTripletRankLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param) {}
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  virtual inline int ExactNumBottomBlobs() const { return 2; }
  virtual inline const char* type() const { return "PairTripletRankLoss"; }
  virtual inline bool AllowForceBackward(const int bottom_index) const {
    return true;
  }
...
template <typename Dtype>
class WeightLossLayer : public LossLayer<Dtype> {
 public:
  explicit WeightLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline int ExactNumBottomBlobs() const { return 1; }
  virtual inline const char* type() const { return "WeightLoss"; }
  virtual inline bool AllowForceBackward(const int bottom_index) const {
    return true;
  }
 ...
 protected:
  ...
  template <typename Dtype>
  class RegulationLossLayer : public LossLayer<Dtype> {
  public:
    explicit RegulationLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param) {}
    void Reshape(const vector<Blob<Dtype>*>& bottom,
			 const vector<Blob<Dtype>*>& top);

    inline int ExactNumBottomBlobs() const { return 1; }
    inline const char* type() const { return "RegulationLoss"; }

  protected:
    void Forward_cpu(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top);
    void Forward_gpu(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top){Forward_cpu(bottom, top);};
  ...
  template <typename Dtype>
  class CrossModalLossLayer : public LossLayer<Dtype> {
  public:
    explicit CrossModalLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param) {}
    void Reshape(const vector<Blob<Dtype>*>& bottom,
			 const vector<Blob<Dtype>*>& top);
    ...
    inline const char* type() const { return "CrossModalLoss"; }

    inline bool AllowForceBackward(const int bottom_index) const {
      return true;
    }
    inline void set_phase(const string phase) {phase_ = phase;}
  ...
  template <typename Dtype>
  class CrossModalFalseLossLayer : public LossLayer<Dtype> {
  public:
    explicit CrossModalFalseLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param) {}
    void Reshape(const vector<Blob<Dtype>*>& bottom,
		 const vector<Blob<Dtype>*>& top);
    ...
    inline int ExactNumTopBlobs() const { return 2; }
    inline int ExactNumBottomBlobs() const { return 3; }
    inline const char* type() const { return "CrossModalFalseLoss"; }
  ...
  template <typename Dtype>
  class TripleEuclideanMaxLossLayer : public LossLayer<Dtype> {
  public:
    explicit TripleEuclideanMaxLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param) {}
    void Reshape(const vector<Blob<Dtype>*>& bottom,
                 const vector<Blob<Dtype>*>& top);
    ...
    inline int ExactNumTopBlobs() const { return 2; }
    inline int ExactNumBottomBlobs() const { return 3; }
    inline const char* type() const { return "TripleEuclideanMaxLoss"; }
