...
void Solver<Dtype>::InitTestNets() {
  ...
  void Solver<Dtype>::Step(int iters) {
    ...
    while (iter_ < stop_iter) {
	  ...
	  && Caffe::root_solver()) {
	LOG(INFO) << "the iter is " << iter_ << ", now start testall";
	TestAll();
	LOG(INFO) << "testall is done";
	...
	callbacks_[i]->on_start();
      ...
      Dtype loss = 0;
      if(net_->name() == "GANNet") {// use for GANNet
	if(iter_ % 5 < 1) {
          net_->set_params_lr("thief");
	  net_->layer_by_name("I--->T_loss")->set_phase("thief");
	  net_->layer_by_name("I--->T_loss(f)")->set_phase("thief");
	  net_->layer_by_name("loss_text")->set_phase("thief");
	  net_->layer_by_name("loss_f")->set_phase("thief");
	}
	else {
	  net_->set_params_lr("police");
	  net_->layer_by_name("I--->T_loss")->set_phase("police");
	  net_->layer_by_name("I--->T_loss(f)")->set_phase("police");
          net_->layer_by_name("loss_text")->set_phase("police");
	  net_->layer_by_name("loss_f")->set_phase("police");
	}
      }
...
void Solver<Dtype>::TestAll() {
  ...
  void Solver<Dtype>::Test(const int test_net_id) {
	...
	for (int j = 0; j < result.size(); ++j) {
	  const Dtype* result_vec = result[j]->cpu_data();
	  if(result[j]->count()/result[j]->num() > 3)
	    continue;
	  for (int k = 0; k < result[j]->count(); ++k) {
