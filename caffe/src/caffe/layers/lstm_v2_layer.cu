...
__device__ Dtype cuda_sigmoid(Dtype x) {
  return fabs(x) < Dtype(5) ? (1. / (1. + exp(-x))) : (x > 0 ? Dtype(1) : Dtype(0));
}
...
__device__ Dtype cuda_tanh_diff(Dtype x) {
  // return (1. - x * x);
  // return x * (1. - x);
  return fabs(x) > 0 ? Dtype(1) : Dtype(0);
}
...
template <typename Dtype>
void LstmV2Layer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void LstmV2Layer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
...
INSTANTIATE_LAYER_GPU_FUNCS(LstmV2Layer);
