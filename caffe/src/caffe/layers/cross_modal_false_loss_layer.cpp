...
namespace caffe {
  ...
  void CrossModalFalseLossLayer<Dtype>::Reshape(
  ...
  template <typename Dtype>
  Dtype CrossModalFalseLossLayer<Dtype>::Similarity(
  ...
  template <typename Dtype>
  void CrossModalFalseLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  ...
  template <typename Dtype>
  void CrossModalFalseLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top, const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) {
  }

#ifdef CPU_ONLY
  STUB_GPU(CrossModalFalseLossLayer);
#endif

  INSTANTIATE_CLASS(CrossModalFalseLossLayer);
  REGISTER_LAYER_CLASS(CrossModalFalseLoss);
