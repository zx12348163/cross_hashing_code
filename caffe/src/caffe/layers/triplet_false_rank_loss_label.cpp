...
namespace caffe {
  ...
  void TripletFalseRankLossLabelLayer<Dtype>::Reshape(
  ...
  template <typename Dtype>
  Dtype TripletFalseRankLossLabelLayer<Dtype>::Similarity(
  ...
  template <typename Dtype>
  void TripletFalseRankLossLabelLayer<Dtype>::Forward_cpu(
    ...
    for (int i = 0; i < num; ++i) {
		...
		Dtype sim = Similarity(label, i, j,  label_dim, bottom[1]->num());
      	...
      	std::sort(sim_vector.begin(), sim_vector.end());
      	//Dtype best_dcg = 0.0;
  ...
  template <typename Dtype>
  void TripletFalseRankLossLabelLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
    ...
    if(n_tri > 0) {
      if (phase_ == "police") caffe_scal(bottom[0]->count(), Dtype(0), diff);
      caffe_scal(bottom[0]->count(), loss_weight / n_tri / margin, diff);
    }
  }

#ifdef CPU_ONLY
  STUB_GPU(TripletFalseRankLossLabelLayer);
#endif

  INSTANTIATE_CLASS(TripletFalseRankLossLabelLayer);
  REGISTER_LAYER_CLASS(TripletFalseRankLossLabel);
