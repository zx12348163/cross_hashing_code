/********
Line:
(4+)49, 109, 154, 162, 167, 235, 475, 461, 519, 519, 461
 *******/
#include <algorithm>
#include <map>
#include <set>
...
void Net<Dtype>::Init(const NetParameter& in_param) {
  LOG(INFO) << "Initialize network, omit LOGs and queue empty info in blocking_queue.cpp, line 50";
  ...
  //std::cout<<std::endl;
  //LOG(INFO)<<"blobs_ : "<<blobs_.size();
  //for (int i = 0; i < blobs_.size(); i ++) {
  //  LOG(INFO)<<blobs_[i]->count();
  //}
  //std::cout<<std::endl;
  /*************************************
  int bn, bc, bh, bw;
  int tn, tc, th, tw;
  LOG(INFO)<<"bottom_size (equals how many layers): " << bottom_vecs_.size();
  LOG(INFO)<<"top_size (equals how many layers): " << top_vecs_.size();
  for (int i = 0; i < bottom_vecs_.size(); i ++) {
    LOG(INFO)<< layer_names_[i]<<" layer has " << bottom_vecs_[i].size()
         <<" bottom_blobs, "<<top_vecs_[i].size()<<" top_blobs.";
    for (int j = 0; j < bottom_vecs_[i].size(); j ++) {
      bn = bottom_vecs_[i][j]->num();
      bc = bottom_vecs_[i][j]->channels();
      bh = bottom_vecs_[i][j]->height();
      bw = bottom_vecs_[i][j]->width();
      LOG(INFO)<<"the "<<j<<"th bottom_blob with "
	       <<bn<<" "<<bc<<" "<<bh<<" "<<bw;
    }
    for (int j = 0; j < top_vecs_[i].size(); j ++) {
      tn = top_vecs_[i][j]->num();
      tc = top_vecs_[i][j]->channels();
      th = top_vecs_[i][j]->height();
      tw = top_vecs_[i][j]->width();
      LOG(INFO)<<"the "<<j<<"th top_blob with "
               <<tn<<" "<<tc<<" "<<th<<" "<<tw;
    }
    std::cout<<std::endl;
  }
  LOG(INFO)<<"All elements of the whole blobs are 0";
  // std::cout<<std::endl;
  //LOG(INFO)<< "params:";
  //for (int i = 0; i < params_.size(); i ++) {
  // std::cout << "param : " << i << ",  ";
  // std::cout << "size is : " << params_[i]->num() << " " << params_[i]->channels()
  //	      << " " << params_[i]->height() << " " << params_[i]->width() << std::endl;
  //}
  int iii;
  std::cin >> iii;
  ********************************************/
...
Dtype Net<Dtype>::ForwardFromTo(int start, int end) {
 ...
 for (int i = start; i <= end; ++i) {
   string tmp_name = layers_[i]->layer_param().name();    
   string tmp_type = layers_[i]->layer_param().type();
   //LOG(INFO)<<"layer :"<<tmp_name<<", type :"<<layers_[i]->type();
   Dtype layer_loss = layers_[i]->Forward(bottom_vecs_[i], top_vecs_[i]);
   loss += layer_loss;
   int N,C,H,W;
   if(tmp_name == "loss" || tmp_name == "loss_v") {
     /**  
     N = top_vecs_[i][0]->num();
     C = top_vecs_[i][0]->channels();
     H = top_vecs_[i][0]->height();
     W = top_vecs_[i][0]->width();
     LOG(INFO) << "num,channels,height,width: " << N << "," << C << "," << H << "," << W;
     int iii;
     std::cin >> iii;
   for (int k = 0; k < N; k ++) {
     for (int j = 0; j < C; j ++) {
       for (int x = 0; x < H; x ++) {
	 for (int y = 0; y < W; y ++) {
	   LOG(INFO) << k << ", " << j << ", " << x << ", " << y << " : " << top_vecs_[i][0]->data_at(k, j, x, y);
	 }
       }
     }
     }/*****/
  ...
  // LOG(INFO)<<"complete once forward computation of all layers";
  /**
  int lay = 0;
  LOG(INFO) << "layer " << lay;
    for (int s = 0; s < top_vecs_[lay][0]->num(); s ++) {
        LOG(INFO) << "batch :" << s;
	for (int t = 0; t < top_vecs_[lay][0]->channels(); t ++) { 
	  for (int x = 0; x < top_vecs_[lay][0]->height(); x ++){
	    for(int y = 0; y < top_vecs_[lay][0]->width(); y ++){
	      double tmp = (double)top_vecs_[lay][0]->data_at(s, t, x, y);
	      if (tmp > 0) std::cout << " ";
	      else std::cout << "*"; 
	    }
	    std::cout << endl;
	...
	std::cout << "the label is " << top_vecs_[lay][1]->data_at(s,0,0,0) << std::endl;
    int iii;
    std::cin>>iii;
    }
/*****************************************************************/
...
void Net<Dtype>::BackwardFromTo(int start, int end) {
  ...
  for (int i = start; i >= end; --i) {
    if (layer_need_backward_[i]) {
      ...
      //LOG(INFO) << layers_[i]->layer_param().type();
      layers_[i]->Backward(
...
void Net<Dtype>::Update() {
  for (int i = 0; i < learnable_params_.size(); ++i) {
    if(name_ == "GANNet") {
      if(params_lr_[i] == 0) continue;// added by zx
    }
...
template <typename Dtype>
void Net<Dtype>::set_params_lr(const string phase) {
  //for vgg net, img_mask = 32,33; bows_mask = 46,47
  //for vgg_f net, img_mask = 10,11; bows_mask = 24,25
  if (phase == "thief") {
    LOG(INFO) << "set phase: adversary_mask";
    for (int i = 0; i < params_lr_.size(); ++ i) {
      if (i == 46) params_lr_[i] = 1;
      else if (i == 47) params_lr_[i] = 2;
      else params_lr_[i] = 0;
    }
  }
  else if (phase == "police") {
    LOG(INFO) << "set phase: cross_modal_retrieval";
    for (int i = 0; i < params_lr_.size(); ++ i) {
      if (i == 46 || i == 47) params_lr_[i] = 0;
      else params_lr_[i] = i % 2 + 1;
    }
